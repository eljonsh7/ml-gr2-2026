...
def remove_outliers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Exclude non-feature columns from outlier detection logic if necessary
    candidates = [c for c in numeric_columns if c not in ["is_weekend", "hour", "day_of_week", "month", "day"]]
...
def build_model_matrix(df: pd.DataFrame, target_column: str) -> tuple[pd.DataFrame, pd.Series]:
    # Drop Datetime or other non-feature columns
    cols_to_drop = [target_column]
    if "Datetime (UTC)" in df.columns: cols_to_drop.append("Datetime (UTC)")
...
def main():
    ...
    print(f"Step 4 completed. Train size: {len(y_train_bal)} rows")
